...
namespace AccessControlCardCHT
{
    ...
    /// 匯入班級
    /// </summary>
    public class ImportAccessControlCard : ImportWizard
    {
        ...
        //須更新的學生系統編號
        //List<int> _DoUpdateIDs = new List<int>();
        ...
        public ImportAccessControlCard()
        {
            this.IsSplit = false;
        }
        ...
        {
            //if (this.ImportOption.SelectedKeyFields.Count > 0)
            //{
            //    string Key = this.ImportOption.SelectedKeyFields[0];

            //    if (Key.Equals("學生系統編號"))
            //    {
            //        return Properties.Resources.AccessControlCard_StudentID;
            //    }
            //    else if (Key.Equals("學號"))
            //    {
            //        return Properties.Resources.AccessControlCard_StudentNumber;
            //    }
            //    else
            //        return Properties.Resources.AccessControlCard;
            //}else
            return Properties.Resources.AccessControlCard;
        }
        ...
        public override void Prepare(ImportOption Option)
        {
            ...
            //DataTable table = mQueryHelper.Select("select ref_student_id from $cht_access_control_card.student_cardno");
            ...
            foreach (DataRow row in table.Rows)
            {
                ...
                string number = row["student_number"].ToString();

                if (!mStudentNumberIDs.ContainsKey(number) && !string.IsNullOrWhiteSpace(number))
                {
                    mStudentNumberIDs.Add(number, id);
                }
            }
        ...
        public override string Import(List<Campus.DocumentValidator.IRowStream> Rows)
        {
            ...
            foreach (IRowStream Row in Rows)
            {
                ...
                if (mStudentNumberIDs.ContainsKey(StudentNumber))
                {
                    string StudentID = mStudentNumberIDs[StudentNumber];
